...
|---|---|---|---|
| High sys% CPU, throughput flat | syscall-heavy path; lock contention in kernel; page faults; network stack cost | `perf top -g` kernel side; `pidstat -w`; `perf stat -e context-switches,page-faults` | `cgroup-cpu-throttling` `cpu-averages-hide-saturation` `eevdf-vs-cfs` `false-sharing-coherence` `flame-graph-interpretation` `hypothesis-driven-investigation` `irq-softirq-napi` `kernel-spectre-mitigations` `mpstat-sar-cpu-breakdown` `off-cpu-analysis` `perf-stat-record` `pidstat-context-switches` `queueing-theory-basics` `rhel9-rhel10-kernel-delta` `rps-rfs-xps-steering` `thp-transparent-hugepages` `tuned-profiles-rhel` `use-method` |
| CPU not saturated, throughput plateaus with concurrency | userspace lock contention; single contended resource; connection/accept bottleneck; coordinated omission hiding it | off-CPU flame graph; `bpftrace` futex latency; check load generator methodology | `cgroup-cpu-throttling` `cpu-averages-hide-saturation` `cpu-frequency-turbo` `eevdf-vs-cfs` `flame-graph-interpretation` `hypothesis-driven-investigation` `mglru-page-reclaim` `off-cpu-analysis` `pidstat-context-switches` `psi-pressure-stall` `queueing-theory-basics` `rhel9-rhel10-kernel-delta` `tcp-incast` `tuned-profiles-rhel` `use-method` |
...
| Low IPC (<~0.5) on busy cores | memory-bound (LLC misses, DRAM bandwidth); false sharing; TLB pressure | TMA level 1 (`perf stat -M tma_...` / toplev); `perf c2c`; `perf stat -e dTLB-load-misses` | `cpu-frequency-turbo` `false-sharing-coherence` `kernel-spectre-mitigations` `numa-remote-memory` `perf-stat-record` `thp-transparent-hugepages` |
| One core pegged in softirq | IRQ steering imbalance; NAPI overload; RPS/RFS misconfig | `mpstat -P ALL`; `/proc/interrupts` deltas; `/proc/softirqs` | `cpu-averages-hide-saturation` `irq-softirq-napi` `mpstat-sar-cpu-breakdown` `qdisc-ring-drops` `rps-rfs-xps-steering` |
| "System A uses more CPU" concluded from host-wide means | host-wide average masks one pinned CPU; IRQ/softirq placement changed between OS versions; single-stream vs multi-stream analysis mismatch | `mpstat -P ALL` (hottest CPU, not mean); `/proc/interrupts` NIC queue CPU distribution; compare softirq-CPU composition between runs | `cpu-averages-hide-saturation` `irq-softirq-napi` `mpstat-sar-cpu-breakdown` `rps-rfs-xps-steering` |
...
| TTFT high, TPOT fine | prefill queueing; prefix cache misses; scheduler starvation of new requests | vLLM queue/scheduler metrics; request-size distribution; prefix cache hit rate | `tokenization-cpu-bottleneck` `vllm-continuous-batching` `vllm-prefix-cache` |
| TPOT degrades under load | KV cache pressure → preemption/recompute; decode batch too large; memory bandwidth bound | vLLM preemption + cache usage metrics; Nsight Compute on decode kernels | `vllm-continuous-batching` `vllm-kv-cache-paged-attention` `vllm-prefix-cache` |
...
| iowait high, disk "slow" | queue depth mismatch; io scheduler choice; write cache/barrier behavior; actually fine and CPU is waiting on sync semantics | `iostat -x` (aqu-sz, await vs svctm-era intuition traps); `biolatency`; fio baseline of raw device | `biolatency-iostat` `mpstat-sar-cpu-breakdown` `off-cpu-analysis` `psi-pressure-stall` `use-method` |
