...
| iowait high, disk "slow" | queue depth mismatch; io scheduler choice; write cache/barrier behavior; actually fine and CPU is waiting on sync semantics | `iostat -x` (aqu-sz, await vs svctm-era intuition traps); `biolatency`; fio baseline of raw device | TBD |
