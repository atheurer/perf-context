...
|---|---|---|---|
| High sys% CPU, throughput flat | syscall-heavy path; lock contention in kernel; page faults; network stack cost | `perf top -g` kernel side; `pidstat -w`; `perf stat -e context-switches,page-faults` | `eevdf-vs-cfs` `mpstat-sar-cpu-breakdown` `rhel9-rhel10-kernel-delta` |
| CPU not saturated, throughput plateaus with concurrency | userspace lock contention; single contended resource; connection/accept bottleneck; coordinated omission hiding it | off-CPU flame graph; `bpftrace` futex latency; check load generator methodology | `eevdf-vs-cfs` `mglru-page-reclaim` `rhel9-rhel10-kernel-delta` |
...
| One core pegged in softirq | IRQ steering imbalance; NAPI overload; RPS/RFS misconfig | `mpstat -P ALL`; `/proc/interrupts` deltas; `/proc/softirqs` | `mpstat-sar-cpu-breakdown` |
...
| cgroup app slow, host idle | CPU quota throttling; memory.high reclaim pressure; io.max | `cpu.stat` nr_throttled; `memory.stat`; PSI (`/proc/pressure/*`, per-cgroup) | TBD |
...
| Packet drops / retransmits under load | ring buffer overflow; qdisc drops; rmem/wmem limits; incast | `ethtool -S` deltas; `ss -ti`; `nstat`; qdisc stats (`tc -s qdisc`) | TBD |
